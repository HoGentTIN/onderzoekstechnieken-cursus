...
De laatste functie die we onderzoeken is de \texttt{rnorm} functie die willekeurige getallen kan genereren waarvan de distributie normaal is. Het argument dat je ingeeft is het aantal willekeurige getallen dt u wilt, met optionele argumenten om de gemiddelde en standaardafwijking op te geven:
...
\begin{exercise}
  Bereken teken ook elke keer het gevraagde gebied.
  \begin{enumerate}[label=\alph*.]
    \item $P(Z < 1.33)$
    \item $P(Z > 1.33)$
    \item $P(Z < -1.33)$
...
\begin{exercise}
	Gebruik de functie rnorm () om een ​​willekeurige steekproef van 25 waarden uit een normale verdeling te tekenen met
	een gemiddelde van 0 en een standaardafwijking gelijk aan 1,0. Gebruik een histogram, met probability = TRUE.
	Maak een overlay over het histogram met: (a) een geschatte dichtheidscurve; (B) de theoretische
	dichtheidscurve voor een normale verdeling met gemiddelde 0 en standaardafwijking gelijk aan 1,0. Herhaal dit voor een steekproef vna 100 en 500 waarden.
\end{exercise}

%\lstinputlisting{oplossingen/oefn38HistCruve.R}
