...
\begin{enumerate}
  ...
  \item Het significantieniveau $\alpha$ en steekproefomvang $n$ worden vastelegd. Je kan $\alpha$ in principe zelf kiezen (bv. 0,05). Hoe hoger het significantieniveau, hoe betrouwbaarder het resultaat van de toets.
...
\begin{example}
  ...
  De onderzoekers van de superhelden stellen dat een superheld gemiddeld 3,3 mensen per dag redt. Ik zelf heb het gevoel dat dat niet zo is: ik heb de indruk dat een superheld \emph{meer} dan $3,3$ mensen per dag redt.
  ...
  \begin{enumerate}
    \item Ik veronderstel dat het aantal mensen dat een superheld redt normaal verdeeld  is en ik formuleer twee hypothesen omtrent de parameter $\mu$.
    \begin{itemize}
    ...
    \item Vastleggen significantieniveau $\alpha$ en steekproefomvang $n$. Ik wil een significantieniveau van 5\% kiezen, dus neem ik $\alpha = 0,05$. De steekproefomgang is gegeven en is hier $n = 30$.
...
\begin{example}
  ...
  Is er reden om aan te nemen dat het Salmonella-niveau in het ijs groter is dan 0,3 MPN/g? We zullen gebruik maken van de R-functie \texttt{t.test} om deze vraag te beantwoorden. Lees zelf de help-pagina van deze functie om de mogelijke opties te leren kennen.
  ...
  \begin{enumerate}
    ...
    \item Vastleggen significantieniveau $\alpha = 0.05$ en steekproefomvang $n = 9$
    
    \item Bepalen overschrijdingskans. Het gaat hier over een eenzijdig rechtse toets, wat aangegeven wordt met de optie \texttt{alternative="greater"}. Het gekozen significantieniveau is wat de functie standaard neemt, dus moet niet expliciet meegegeven worden.
...
\begin{itemize}
  ...
  \item De steekproeven zijn afhankelijk, of gepaard. Een voorbeeld is twee metingen uitvoeren op eenzelfde lid van de populatie, zoals de koorts nemen voor en na het innemen van een medicijn.
\end{itemize}
...
\begin{exercise}
  ...
  Concreet: ga aan de hand van een toets voor twee steekproeven voor elke datahoeveelheid na of het gemiddelde van het best scorende persistentietype \emph{lager} is dan het gemiddelde van \begin{inparaenum}[(i)] \item het \emph{tweede} beste en \item het slechtst scorende type \end{inparaenum}.
  
  Kunnen we de conclusie aanhouden dat één persistentietype het beste is, d.w.z.~significant beter is dan gelijk welk ander persistentietype?
\end{exercise}

\begin{exercise}
  Een groot aantal studenten heeft deelgenomen aan een test die in verschillende opeenvolgende sessies werd georganiseerd. De opgave was telkens dezelfde, dus eigenlijk bestaat het gevaar dat studenten na de test info konden doorspelen aan de groepen die nog moesten komen. De latere groepen hebben dan eigenlijk een voordeel ten opzichte van de eerste. Blijkt dit ook uit de cijfers?
