...
\begin{enumerate}
  \item Hypothese:
    \begin{itemize}
     \item $H_0$: $\overline{x} - \overline{y} = 0$
     \item $H_1$: $\overline{x} - \overline{y} < 0$
    \end{itemize}
  \item Significantieniveau: $\alpha = 0,05$
  \item Steekproefgrootheid:
    \begin{itemize}
     \item $\overline{x}-\overline{y}$
     \item $\overline{x} = \textnormal{gemiddelde(controlegroep)} = 88,83$ 
     \item $\overline{y} = \textnormal{gemiddelde(interventiegroep)} = 101,67$ 
    \end{itemize}
\end{enumerate}
\vfill
In R:
