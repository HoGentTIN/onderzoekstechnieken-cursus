...
Als er veel verschillende waarden wijn, dan is het aflezen van de modus iets moeilijker. Dan kan je de tabel eventueel sorteren:
...
Het **rekenkundig gemiddelde**  $\mu$ (de Griekse letter mu), is een centrummaat die gedefinieerd is als de som van alle getallen in een verzameling $X = {x_1, ..., x_n}$, gedeeld door het aantal getallen:
...
mu <- sum(x) / length(x)   # Formule toepassen
mean(x)                    # Functie mean() gebruiken
```

### Variantie en standaardafwijking

De **variantie** $\sigma^2$ (de Griekse letter sigma), is een spreidingsmaat die gedefinieerd is als:
...
```{r}
sigma_kwadraat <- sum((x - mu)^2) / length(x)
```

De **standaardafwijking** $\sigma$ is de vierkantswortel van de variantie: $\sigma = \sqrt{\sigma^2}$

```{r}
sqrt(sigma_kwadraat)
```

**Opmerking:** In R bestaat er een functie voor het berekenen van de standaardafwijking, `sd()`, maar die is een beetje anders gedefinieerd. We zullen deze later in de cursus introduceren.
...
Doe hetzelfde voor de variabele `starwars$mass`. Filter eerst de lege waarden er uit! Ter controle: het gemiddelde is (afgerond) 97.3119, de standaardafwijking 168.0149.
