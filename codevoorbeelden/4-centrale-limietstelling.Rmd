...
```{r}
# X-waarden in het interval dat ons interesseert
xvals <- seq(from = m-4*s, to = m+4*s, length.out = 201)
# Welke waarden in dat interval liggen links van x?
i <- xvals > x
# Theoretische dichtheid voor deze X-waarden
dist <- dnorm(xvals, mean = m, sd = s)
dx <- dnorm(x, mean = m, sd = s)

ggplot() +
  # Toon het gebied rechts van x en onder de Gauss-curve in het groen
  geom_polygon(mapping = aes(
    x = c(x, x,  xvals[i]),
    y = c(0, dx, dist[i])
  ), color = "black", fill = "lightgreen") +
  # Teken de theoretische kansdichtheidsfunctie (Gauss-curve)
  geom_line(aes(xvals, dist)) +
  # Toon het gemiddelde als een rode verticale lijn
  geom_vline(xintercept = m, color = "red") +
  xlab("reactiesnelheid") + ylab("kansdichtheid") +
  # Voeg tekst toe aan de plot
  annotate("text", x = x, y = -.01, label = x) +
  annotate("text", x = m+.1, y = -.01, label = m, color = "red")
```

De rechterstaartkans $P(X < 6.5)$ is het groene gebied op deze grafiek:
